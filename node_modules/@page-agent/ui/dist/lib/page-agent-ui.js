...
function createCard({ icon, content, meta, type }) {
  ...
  const contentHtml = Array.isArray(content) ? `<div class="${styles.reflectionLines}">${content.map((line) => `<span>${escapeHtml(line)}</span>`).join("")}</div>` : `<span>${escapeHtml(content)}</span>`;
  return `
...
createWrapper_fn = /* @__PURE__ */ __name(function() {
  ...
  wrapper2.setAttribute("data-page-agent-ignore", "true");
  wrapper2.innerHTML = `
...
renderHistory_fn = /* @__PURE__ */ __name(function() {
  ...
  for (const event of history) {
    items.push(...__privateMethod(this, _Panel_instances, createHistoryCards_fn).call(this, event));
  }
...
createHistoryCards_fn = /* @__PURE__ */ __name(function(event) {
  const cards = [];
  const meta = event.type === "step" && event.stepIndex !== void 0 ? __privateGet(this, _i18n).t("ui.panel.step", {
